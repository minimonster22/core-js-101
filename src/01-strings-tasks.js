...
function extractEmails(str) {
  ...
  const filteredEmails = emailArray.filter((email) => email.trim() !== ''); // Добавлены круглые скобки
...
function getRectangleString(width, height) {
  ...
  const rectangle = horizontalBorder + middlePart.repeat(height - 2) + verticalBorder;
  return rectangle;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    const offset = isLowerCase ? 97 : 65;
