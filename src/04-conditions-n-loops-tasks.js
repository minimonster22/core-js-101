...
function findFirstSingleChar(str) {
  ...
  const firstSingleChar = Array.from(str).find((char) => charCount.get(char) === 1);
...
function isCreditCardNumber(ccn) {
  ...
  function calculateLuhnSum(str, index, double) {
    if (index < 0) {
      return 0;
    }

    let digit = parseInt(str.charAt(index), 10);
    ...
    return digit + calculateLuhnSum(str, index - 1, !double);
  }

  const sum = calculateLuhnSum(ccnStr, ccnStr.length - 1, false);
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    throw new Error('Radix n is out of the supported range');
  }
  ...
  let result = '';
  let number = num; // Используйте другое имя переменной
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonPath = pathes.reduce((acc, path) => {
    const parts = path.split('/');

    let i = 0;
    while (i < parts.length && parts[i] === acc[i]) {
      i += 1;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let j = 0; j < 3; j += 1) {
    if (position[0][j] === position[1][j] && position[1][j] === position[2][j]) {
      if (position[0][j] === 'X' || position[0][j] === '0') {
        return position[0][j];
      }
    }
  }
